...
public class Usuarios {
    ...
    public ArrayList<Usuario> listUsers(){ return convertirUsuariosFicheroEnArrayList(); }

    public Usuario getUser(String username){
        ArrayList<Usuario> usuarios = convertirUsuariosFicheroEnArrayList();

        for (Usuario usuarioActual : usuarios) {
            if (usuarioActual.nombreUsuario.equalsIgnoreCase(username)) { return usuarioActual; }
        }
        return null;
    }

    public void createUser(Usuario usuario){
        if (getUser(usuario.nombreUsuario) != null) {
            return;
        }
        ...
        try {
            ...
            System.out.println("Error typing: " + e.getMessage());
        }
    }
